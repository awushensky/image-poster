import type { CronSchedule, PostingTime, QueuedImage } from "~/model/model";
import { getNextExecutions, getNextExecutionsForMultipleSchedules } from "./cron-utils";
...
export function estimateImageSchedule(
  ...
  return queuedImages.map((image, index) => ({
    ...image,
    estimatedPostTime: executionDates[index] || undefined
  }));
}
